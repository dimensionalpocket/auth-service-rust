...
- When you're asked to write a plan:
  - Save the plan in the `docs/plans` directory. If the directory doesn't exist, create it. Use a directory structure based on the current year and month (`YYYY/MM` format). Name the file with the day only (`DD`), followed by a dash, and then the task name in lowercase with dashes. For example: `2025/04/16-do-something.md`. There's no need to add a `-plan` suffix to the file name.
