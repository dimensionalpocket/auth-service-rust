...
impl RemoveSiteResolver {
  ...
  async fn remove_site(
    ...
    match RemoveSiteOrchestrator::run(databases, session_context.clone(), config, site_id).await {
...
mod tests {
  ...
  async fn test_remove_site_success() {
    ...
    let admin_role_id =
      create_test_role_with_databases(&databases, "admin", &["is_admin", "can_delete_site"]).await;
