...
mod tests {
  ...
  async fn test_update_password_success() {
    ...
    let user = {
      let mut conn = pool.acquire().await.unwrap();
      UserService::create_user(&mut conn, "testuser", "oldpassword123")
        .await
        .unwrap()
    };
  ...
  async fn test_update_password_invalid_current_password() {
    ...
    let user = {
      let mut conn = pool.acquire().await.unwrap();
      UserService::create_user(&mut conn, "testuser", "correctpassword")
        .await
        .unwrap()
    };
  ...
  async fn test_update_password_password_confirmation_mismatch() {
    ...
    let user = {
      let mut conn = pool.acquire().await.unwrap();
      UserService::create_user(&mut conn, "testuser", "currentpassword")
        .await
        .unwrap()
    };
